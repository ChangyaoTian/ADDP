...
class MaskedGenerativeEncoderViT(nn.Module):
    ...
    @torch.no_grad()
    def sample_tokens(self, token_indices, token_all_mask, return_pred_tokens=True, codebook=None):
        codebook_size = 1024
        ...
        b, L, n = logits.shape
        logits = logits.reshape(b, int(L**.5), int(L**.5), n).permute(0, 3, 1, 2)
        
        pred_prob = F.softmax(logits, dim=1)
        if return_pred_tokens:
            pred_tokens = torch.einsum('b n h w, n d -> b d h w', pred_prob, codebook)
            return pred_tokens
        else:
            return pred_prob
...
def mage_vit_tiny_patch16(**kwargs):
    model = MaskedGenerativeEncoderViT(
        patch_size=16, embed_dim=256, depth=12, num_heads=4,
        decoder_embed_dim=256, decoder_depth=8, decoder_num_heads=4,
        mlp_ratio=4, norm_layer=partial(nn.LayerNorm, eps=1e-6), **kwargs)
    return model

def token_predictor(model_name, **kwargs):
    if model_name == 'mage_vit_base_patch16':
        return mage_vit_base_patch16(**kwargs)
    elif model_name == 'mage_vit_tiny_patch16':
        return mage_vit_tiny_patch16(**kwargs)
    elif model_name == 'mage_vit_large_patch16':
        return mage_vit_large_patch16(**kwargs)
