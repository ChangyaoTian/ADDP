...
class Attention(nn.Module):
    def __init__(self, dim, num_heads=8, qkv_bias=False, attn_drop=0., proj_drop=0., attn_fp32=False):
        super().__init__()
...
class Block(nn.Module):
    ...
    def __init__(
            ...
            drop_path=0., act_layer=nn.GELU, norm_layer=LayerNorm, attn_fp32=False):
        super().__init__()
        self.norm1 = norm_layer(dim)
        self.attn = Attention(dim, num_heads=num_heads, qkv_bias=qkv_bias, attn_drop=attn_drop, proj_drop=drop, attn_fp32=attn_fp32)
    ...
    def forward(self, x, sep_bn_ind=None):
        if sep_bn_ind is not None:
            x = x + self.drop_path1(self.ls1(self.attn(self.norm1(x, sep_ind=sep_bn_ind))))
            x = x + self.drop_path2(self.ls2(self.mlp(self.norm2(x, sep_ind=sep_bn_ind))))
        else:
            x = x + self.drop_path1(self.ls1(self.attn(self.norm1(x))))
            # res_x = x
            # x = self.norm1(x)
            # x = torch.utils.checkpoint.checkpoint(self.attn, x)
            # x = res_x + self.drop_path1(self.ls1(x))
            ...
            # x = res_x + self.drop_path2(self.ls2(x))
        return x
...
class VisionTransformer(timm.models.vision_transformer.VisionTransformer):
    ...
    """
    def __init__(self, global_pool='avg', args=None, **kwargs,):
        self.args = args
        ...
        if self.global_pool in ['avg', 'pixel']:
        ...
        pos_embed_requires_grad = True # (args.pos_embed_type == 'learnable')
        if self.global_pool:
            self.pos_embed = nn.Parameter(torch.zeros(1, num_patches+1, embed_dim), requires_grad=pos_embed_requires_grad)
        else:
            self.pos_embed = nn.Parameter(torch.zeros(1, num_patches, embed_dim), requires_grad=pos_embed_requires_grad)
            self.cls_pos_embed = nn.Parameter(torch.zeros(1, 1, embed_dim), requires_grad=pos_embed_requires_grad)
    
    def forward_mlp_tokenizer(self, x):
        B = x.shape[0]
    ...
    def forward_features(self, x):
        x = self.forward_mlp_tokenizer(x)
